...
coupdate :: (k `CoAlters` a) w => X a -> k -> (w a -> Maybe a) -> w (Maybe a)
coupdate p k f = coalter p k $ \wma -> case cosequence wma of
  Just wa -> coalter p k (cosequence $ Just wa)
  Nothing -> wma

coupdate_ :: (k `CoAlters` a) w => X a -> k -> (a -> w (Maybe a)) -> ()
coupdate_ p k = const () $ coupdate p k

coupdateStorefully :: (k `CoAlters` a) w => X a -> k -> StoreT a w (Maybe a) -> Maybe a
coupdateStorefully p k = coupdate p k . peek

coupdateStorefully_ :: (k `CoAlters` a) w => X a -> k -> StoreT a w (Maybe a) -> ()
coupdateStorefully_ p k = const () $ coupdateStorefully p k

coadjust :: (k `CoAlters` a) w => X a -> k -> (w a -> a) -> a
coadjust p k w = fromJust $ coupdate p k (Just . f)

coadjust_ :: (k `CoAlters` a) w => X a -> k -> (w a -> a) -> ()
coadjust_ p k = const () $ coadjust p k

coadjustStatefully :: (k `CoAlters` a) w => X a -> k -> StoreT a w () -> a
coadjustStatefully p k = coadjust p k . pos

coadjustStorefully_ :: (k `CoAlters` a) w => X a -> k -> StoreT a w () -> ()
coadjustStorefully_ p k = const () $ coadjustStorefully p k

corepsert :: (k `CoAlters` a) w => X a -> k -> (w (Maybe a) -> a) -> a
corepsert p k w = fromJust $ coalter p k (Just . f)

corepsert_ :: (k `CoAlters` a) w => X a -> k -> (w (Maybe a) -> a) -> ()
corepsert_ p k = const () $ corepsert p k

colookup :: (k `CoAlters` a) w => X a -> k -> Maybe a
colookup p k = coalter p k extract

coinsert :: (k `CoAlters` a) w => X a -> k -> a -> ()
